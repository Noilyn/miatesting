...
# Save Settings
FILE_DIR = os.path.abspath(os.path.abspath(__file__))
...
def parse_arguments():
    ...
    parser.add_argument('--method', type=str, default='relaxloss', help='method name',
                        choices=['vanilla', 'relaxloss', 'advreg', 'dpsgd', 'confidence_penalty',
                                 'distillation', 'dropout', 'eraly_stopping', 'label_smoothing'])
    parser.add_argument('--seed', '-s', type=int, default=1000, help='random seed')
    parser.add_argument('--mode', type=str, default='defense_attack', help='mode of the process '
                        'to be run', choices=['shadow', 'defense', 'attack', 'defense_attack'])
...
def run_defense(dataset_prefix, save_root, args, method):
    ...
    if method == 'distillation':
        ...
        if not os.path.exists(os.path.join(teacher_path, 'model.pt')):
            command = f'python {dataset_prefix}/defense/vanilla.py -name seed{args.seed} -s ' \
                      f'{args.seed} --dataset {args.dataset}'
            command += f' --model {args.model}' if model_flag else ''
            os.system(command)

        ## train student model
        command = f'python {dataset_prefix}/denfense/{method}.py -name seed{args.seed} --dataset ' \
                  f'{args.dataset} -teacher {teacher_path}'
        command += f' --model {args.model}' if model_flag else ''
        os.system(command)

    else:
        command = f'python {dataset_prefix}/defense/{method}.py -name seed{args.seed} --dataset ' \
                  f'{args.dataset}'
        command += f' --model {args.model}' if model_flag else ''
        os.system(command)
...
def run_shadow(dataset_prefix, save_root, args, method):
    ...
    if method == 'distillation':
        ...
        if not os.path.exists(os.path.join(teacher_path, 'model.pt')):
            command = f'python {dataset_prefix}/denfense/vanilla.py -name seed{args.seed} --dataset ' \
                    f'{args.seed} --dataset {args.dataset} --partition shadow'
            command += f' --model {args.model}' if model_flag else ''
            os.system(command)
        ...
        command = f'python {dataset_prefix}/defense/{method}.py -name seed{args.seed} -s ' \
                  f'{args.seed} --dataset {args.dataset} --partition shadow'
        command += f' --model {args.model}' if model_flag else ''
        os.system(command)
...
def run_attack(dataset_prefix, target, shadow):
    os.system(f'python {dataset_prefix}/run_attack.py -target {target} -shadow {shadow}')
...
def main():
    ...
    if args.mode == 'attack':
        ...
        if args.method == 'early_stopping':
            all_targets = [p for p in os.listdir(target_path) if os.path.isdir(os.path.join(
                target_path, p)) and 'ep' in p]
            for target_path in all_targets:
                run_attack(dataset_prefix, target_path, base_shadow_path)
        else:
            run_attack(dataset_prefix, target_path, base_shadow_path)

    if args.mode == 'defense_attack':
        run_defense(dataset_prefix, save_root, args, args.method)
