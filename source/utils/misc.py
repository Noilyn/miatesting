...
__all__ = ['mkdir', 'savefig', 'plot_hist', 'str2bool', 'get_all_losses', 'load_yaml',
           'write_yaml', 'savepickle', 'unpickle', 'write_csv', 'Partition']
...
def plot_hsit(values, names, save_file):
    plt.figure()
...
def write_yml(data, fpath):
    with open(fpath, 'w') as f:
        yaml.dump(data, f)


def savepickle(data, fpath):
...
def write_csv(fpath, entry_title, entry_data, index_names=None):
            ...
            for k, v in d.items():
                if k in self.keys():
                    if v is not None and self[k] is not None:
                        print(f'{k} is already defined to be {self[k]}. Overwritting to {v}')
                    if v is None:
                        continue
                k = k.replace('-', '_')
                self[k] = v
...
class Partition():
    def __init__(self, dataset_size, indices, target_train_size=None, target_test_size=None,
                 shadow_train_size=None, shadow_test_size=None, pseudo_test_size=None):
        ## Config
        ...
        ## Size of each subset
        self.target_train_size = target_train_size if target_train_size is not None else \
            self.slice_size
        ...
        self.names = ['target_train', 'target_test', 'shadow_train', 'shadow_test', 'pseudo_test']
        self.size_list = [self.target_train_size, self.target_test_size, self.shadow_train_size,
                          self.shadow_test_size, self.pseudo_test_size]
        self.indices_dict = {}
        start_idx = 0
