...
class BaseTrainer(object):
    ...
    def set_dataloader(self):
        ...
        self.transofrm_test = None

        self.partition = None
        self.trainset_idx = None
        self.testest_idx = None
    ...
    def set_logger(self):
        ...
        logger.set_names((['LR', 'Train Loss', 'Val Loss', 'Train Acc', 'Val Acc', 'Train Acc 5',
                           'Val Acc 5']))
        self.logger = logger
    ...
    def test(self, model):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(self.testloader):
                ...
                prec1, prec5 = accuracy(outputs.data, targets.data, topk=(1, 5))
                losses.update(loss.item(), inputs.size(0))
                top1.update(top1.item(), inputs.size(0))
                top5.update(top5.item(), inputs.size(0))

                ### Record the total tiem for processing the batch
                batch_time.update(time.time() - time_stamp)
                time_stamp = time.time()
    ...
    def get_loss_distributions(self, model):
        ...
        test_losses = get_all_losses(self.testloader, model, self.crossentropy_noreduce,
                                     self.device)
